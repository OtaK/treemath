...
libfuzzer_sys::fuzz_target!(|input: Input| {
    treemath::copath_unchecked(input.node_index_constrained, input.leaf_count_constrained);
    treemath::copath(input.node_index_unconstrained, input.leaf_count_unconstrained);
});
