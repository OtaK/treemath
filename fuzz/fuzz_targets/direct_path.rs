...
libfuzzer_sys::fuzz_target!(|input: Input| {
    treemath::direct_path_unchecked(input.node_index_constrained, input.leaf_count_constrained);
    treemath::direct_path(input.node_index_unconstrained, input.leaf_count_unconstrained);
});
